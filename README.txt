...
я открыл несколько интиресных слушателя событий который могут помочь 
1. Слушатель изминения состоянии обкта (то есть мы можем отслеживать изминения в тексте на лету в стилях в клссах атрибутах)
2. Слушатель скрола( слушать где мы находимся относительно обекта и что то делать к примеру добавлять классы)
3. Слушатель шрины экрана ( на каком разрешении мы находимя или на каком устройстве)

// Создаем объект MediaQueryList и указываем на что ему реагировать где точка тригера
const mediaQuery = window.matchMedia("(max-width: 640px)");

matches -- обзначает достигло ли нужной точки если да то тру нет фолс 

// Функция для выполнения условий
function checkMediaQuery(mediaQuery) {
  if (mediaQuery.matches) {
    // Ширина экрана меньше 640px
    // Выполняйте действия для маленьких экранов
    console.log('Ширина экрана меньше 640px.');
  } else {
    // Ширина экрана больше 640px
    // Выполняйте действия для больших экранов
    console.log('Ширина экрана больше 640px.');
  }
}

а дальше можно это обработать мы выше или ниже нашей точки 

// Проверяем MediaQueryList и добавляем слушатель события
просто вызов функици  можно и без нёё так как функция все равно вызываеться при слушателе
checkMediaQuery(mediaQuery);

сам слушатель событий изминения ширины экрана котоырй срабатывает на моменте уазнйо точки 
