...
// Закрытие по нажатию куда угодно кроме хедера
export function menuCloseBackDrop() {
	document.addEventListener("click", function (e) {
		if (document.querySelector(".menu-open")?.classList.contains("menu-open") && !e.target.closest('.header')) {
			menuClose()
		}
	})
}
...
export function formValidationMini() {
	...
	function vaidation(e) {
		...
		if (e.target == formBtn) {
			...
			if ((inpytName.value.length > 2) && (emailRegex.test(inputEmail.value))) {
				...
				setTimeout(function () { snecbarGood.className = snecbarGood.className.replace("show", ""); }, 3000);
			} else {
